...
function snapshot(view, previous) {
  ...
  // A check against the remote stays meaningful only while HEAD (which a detached
  // checkout moves without touching any ref) and every branch tip and
  // remote-tracking ref stay put. A snapshot from before refsHash drops its check once.
  const verified = view.verified !== undefined
    ? view.verified
    : previous?.verified && view.refsHash && previous.head === view.head && previous.refsHash === view.refsHash ? previous.verified : null;
